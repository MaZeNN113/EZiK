...
class AssistActivity : Activity() {
    ...
    private fun installImeHandling(root: View) {
        // Never mutate WindowManager.LayoutParams from the IME insets callback.
        // On MIUI/vendor builds that can trigger another insets pass, which creates
        // the keyboard/window bounce seen as rapid up/down flicker.
        ...
        root.setOnApplyWindowInsetsListener { _, insets ->
            ...
            val extraBottom = (imeBottom - navBottom).coerceAtLeast(0)
            root.setPadding(initialLeft, initialTop, initialRight, initialBottom + extraBottom)
            insets
        }
        root.requestApplyInsets()
    }

    /** System Back, including the Android edge-back gesture, dismisses EZiK. */
